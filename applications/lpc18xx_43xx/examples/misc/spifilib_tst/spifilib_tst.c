...
static SPIFI_HANDLE_T *initializeSpifi(void)
{
	...
	for (idx = 0; idx < spifiGetSuppFamilyCount(); ++idx) {
		DEBUGOUT("Supported family: '%s'\r\n", spifiGetSuppFamilyName(idx));
	}
...
{
	const char *finalRateMessage = "SPIFI final Rate      = %d\r\n";
	...
	/* Setup SPIFI clock to run around 15Mhz. Use divider E for this, as it allows
	   higher divider values up to 256 maximum) */
	Chip_Clock_SetDivider(CLK_IDIV_E, CLKIN_MAINPLL, CalculateDivider(spifiBaseClockRate, 15000000));
	...
	spifiCaps = spifiDevGetInfo(pSpifi, SPIFI_INFO_CAPS);
	...
	DEBUGOUT("Max Read Clock Rate    = %d\r\n", spifiDevGetInfo(pSpifi, SPIFI_INFO_MAX_READ_CLOCK));
	DEBUGOUT("Max HS Read Clock Rate = %d\r\n", spifiDevGetInfo(pSpifi, SPIFI_INFO_MAX_HSREAD_CLOCK));
	DEBUGOUT("Max Prog Clock Rate    = %d\r\n", spifiDevGetInfo(pSpifi, SPIFI_INFO_MAX_PROG_CLOCK));
	DEBUGOUT("Max HS Prog Clock Rate = %d\r\n", spifiDevGetInfo(pSpifi, SPIFI_INFO_MAX_HSPROG_CLOCK));
	...
	if (maxSpifiClock > MAX_SPIFI_CLOCK) {
		...
		finalRateMessage = "SPIFI reduced Rate     = %d\r\n";
	}
	...
	DEBUGOUT(finalRateMessage, Chip_Clock_GetClockInputHz(CLKIN_IDIVE));
	DEBUGOUT("\r\n");
	...
	/* test data integrity */
	test_suiteDataBattery(pSpifi, 0,0);
	if (spifiCaps & SPIFI_CAP_DUAL_READ) {
		test_suiteDataBattery(pSpifi, (ENABLE_DUAL_READ),0);
	}
	if (spifiCaps & SPIFI_CAP_DUAL_WRITE) {
		test_suiteDataBattery(pSpifi, 0,(ENABLE_DUAL_WRITE));
	}
	if ((spifiCaps & SPIFI_CAP_DUAL_READ) && (spifiCaps & SPIFI_CAP_DUAL_WRITE)) {
		test_suiteDataBattery(pSpifi, ENABLE_DUAL_READ, ENABLE_DUAL_WRITE);
	}
	if (spifiCaps & SPIFI_CAP_QUAD_READ) {
		test_suiteDataBattery(pSpifi, (ENABLE_QUAD_READ),0);
	}
	if (spifiCaps & SPIFI_CAP_QUAD_WRITE) {
		test_suiteDataBattery(pSpifi, 0,(ENABLE_QUAD_WRITE));
	}
	if ((spifiCaps & SPIFI_CAP_QUAD_READ) && (spifiCaps & SPIFI_CAP_QUAD_WRITE)) {
		test_suiteDataBattery(pSpifi, ENABLE_QUAD_READ, ENABLE_QUAD_WRITE);
	}

	/* Run the device lock unlock test */
	test_suiteLockUnlockBattery(pSpifi);

	/* Run the performance test */
	test_suitePerformanceBattery(pSpifi);

	/* Run the deInit test */
	test_suiteDeInitBattery(pSpifi);
