...
int main(void)
{
	...
	SystemCoreClockUpdate();
        /* Increase SPIFI clock to greater than 50 MHz to avoid 
            weird issues during SD bulk writes */
#if defined(BOARD_NGX_XPLORER_4330) || defined(BOARD_HITEX_EVA_4350)
        Chip_Clock_SetDivider(CLK_IDIV_E, CLKIN_MAINPLL, 4);
#elif defined(BOARD_NGX_XPLORER_1830)
	...
	/*First initialize SD/MMC and wait for card detect */
	card_init();
	...
	if (ret == LPC_OK) {
		...
		if (ret == LPC_OK) {
			/*  enable USB interrrupts, USB priority should be lower than SD */
