...
typedef struct {
	...
	SemaphoreHandle_t xtx_count_sem;				/**< TX free buffer counting semaphore */
#endif
} lpc_enetdata_t;
...
{
	/* Assume the default PHY MII address. */
	uint8_t phyAddress = LPC_PHYDEF_PHYADDR;
	lpc_enetdata_t *lpc_enetif = netif->state;
	err_t err = ERR_OK;
#if defined(USE_RMII)
	bool useRmii = true;
	...
	/* Initialize the PHY */
	Chip_ENET_SetupMII(LPC_ETHERNET, Chip_ENET_FindMIIDiv(LPC_ETHERNET, 2500000), phyAddress);
	if (lpc_phy_init(useRmii, msDelay) != SUCCESS) {

		/******************************************************************
		 * On the RDB4078 there is a hardware error.  In a normal reset the
		 * LPC4078 pulls its non pull-up IO pins high momentarily.  This
		 * will drive a 1 to the PHYAD0 pin on the PHY, selecting address 1.
		 * But in a debug generated reset on an LPC4078 this pull-up is not 
		 * as strong. This could result that the PHYAD0 pin will not be be 
		 * pulled up fully and the PHY will select address 0. This might be 
		 * the reason lpc_phy_init() failed in the prior statement.  If 
		 * running on an RDB4078, try again using the alternate address.
		 ******************************************************************/
		#ifdef BOARD_RDB4078
		/* Try the alternate PHY address. */
		phyAddress = 0;
		Chip_ENET_SetupMII(LPC_ETHERNET, Chip_ENET_FindMIIDiv(LPC_ETHERNET, 
			2500000), phyAddress);
		if (lpc_phy_init(useRmii, msDelay) != SUCCESS) {
			/* Give up.  Something else must be wrong. */
			return ERROR;
		}
		#else
		return ERROR;
